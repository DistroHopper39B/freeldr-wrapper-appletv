...
void ClearScreen(int alpha) {
    memset((void *) BootStruct->Video.BaseAddress, alpha, BootStruct->Video.Pitch * BootStruct->Video.Height);
...
void PlacePixel(u32 PixelLocationX, u32 PixelLocationY, u32 RgbaValue) {
    ...
    u32 PixelStartingAddr = BootStruct->Video.BaseAddress + ((PixelLocationX - 1) * 4) + ((PixelLocationY - 1) * BootStruct->Video.Pitch);
